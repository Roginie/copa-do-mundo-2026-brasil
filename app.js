/* ============================================================
   ...
   Dados reais embutidos + enriquecimento via TheSportsDB
...
const BASE = {
  ...
  // Histórico H2H (oficiais) — vitórias/empates/derrotas e gols do Brasil
  h2h: {
  ...
  // Aproveitamento dos últimos 10 jogos
...
function renderJogos(jogos) {
  ...
  // índice de assertividade (acertos de vencedor / jogos finalizados)
...
async function buscarResultadosAPI() {
  try {
    ...
    await res.json(); // ponto de extensão: enriquecer placares ao vivo quando a Copa 2026 estiver na API
  } catch {
    /* silencioso — os dados base já cobrem os jogos */
  }
}
